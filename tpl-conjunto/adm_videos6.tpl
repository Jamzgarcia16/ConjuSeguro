  ...
  <!-- Otra Prueba!! -->
    ...
    <section>
      ...
      <table class="table">
        ...
        <?
        {foreach $listPer as $fila}

          <tr>
      <td>{$fila.id_personaje}</td>
      <td>{$fila.nombre}</td>
      <td>{$fila.nombre}</td>
      <td><audio controls><source src="data:audio/mp3;base64,'.base64_encode{$fila.poder}.'"></audio controls></td>
      <td>{$fila.pais}</td>
  </tr>
  {/foreach}
      </table>
